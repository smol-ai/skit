...
Source-derived Collection Identity also changes the `collectionRef` of authored SKITs installed from GitHub, generic Git, or local paths from `declared-skit` to the corresponding source profile. This is intentional: portable artifact metadata no longer overrides acquisition identity. Any development Library state created before the decision is rebuilt rather than migrated.
