...
import { executePortablePinEffect } from "../src/workflows/library/portable-pin.js";
import { addPortableLibrarySourceEffect } from "../src/workflows/library/portable-add.js";
import {
  archiveBytes,
...
it.effect("selects a retained Version and retries Projection without acquiring a Release", () =>
  Effect.gen(function* () {
    ...
    const workspace = yield* fs.makeTempDirectoryScoped({ prefix: "skit-portable-pin-" });
    ...
    const plan = yield* executePortablePinEffect(beforePin, options).pipe(Effect.provide(layer));
    ...
    const apply = Effect.gen(function* () {
      ...
      assert.ok(current);
      return yield* executePortablePinEffect(current, { ...options, dryRun: false }).pipe(
...
it.effect("previews and retains an exact historical Registry release before selecting it", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const workspace = yield* fs.makeTempDirectoryScoped({ prefix: "skit-portable-pin-release-" });
    ...
    yield* fixture.owned(addPortableLibrarySourceEffect({}, "skit:alice/tools", "2.0.0"));
    const before = yield* fixture.durable;
    const collection = before.collections[0]!;
    const preview = yield* fixture.owned(
      executePortablePinEffect(before, {
        ...base,
