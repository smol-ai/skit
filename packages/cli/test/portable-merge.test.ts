...
import { mergePortableManifests } from "../src/workflows/library/portable-merge.js";
import { planPortableLibrarySync } from "../src/workflows/library/portable-sync-plan.js";
import { deferredPortableBindings } from "../src/workflows/library/portable-sync.js";

const digest = `sha256:${"a".repeat(64)}`;
const digestB = `sha256:${"b".repeat(64)}`;
const digestC = `sha256:${"c".repeat(64)}`;
const decode = Schema.decodeUnknownEffect(PortableLibraryManifest);
...
it.effect("merges an independently added global Binding", () =>
  Effect.gen(function* () {
    ...
    const merged = mergePortableManifests(base, local, base);
...
it.effect("reports concurrent Skill selection changes without choosing a winner", () =>
  Effect.gen(function* () {
    ...
    const conflicted = mergePortableManifests(base, local, remote);
    assert.deepEqual(conflicted.conflicts, [`skill:${skillId}`]);
    const accepted = mergePortableManifests(base, local, remote, new Set([`skill:${skillId}`]));
...
it.effect("plans destination-specific Collection and Binding reconciliation", () =>
  Effect.gen(function* () {
    ...
    const plan = planPortableLibrarySync(desired, remote, desired);
